      ...
      <div className="container mx-auto px-6 md:px-10 lg:px-14 py-16 md:py-24">
        ...
        <div className="mb-12 md:mb-16">
          ...
          <h2 className="mt-6 text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight leading-tight">
            Purpose-led. <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#115D8E] to-[#046eb4]">Outcome-driven.</span>
          </h2>
          <p className="mt-4 max-w-3xl text-gray-600">
            We fuse strategy, design, and engineering so every initiative ships fast, scales safely, and delights customers.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-10">
          ...
          <div className="lg:col-span-3">
            <GradientCard>
              ...
              <p className="mt-3 text-gray-600">
                Every interaction—digital or physical—should be intuitive, trusted, and measurably valuable.
              </p>

              <ul className="mt-6 space-y-3">
                {[
                  "Human-centered journeys across all touchpoints",
                  "Trust by default: privacy, security, reliability",
                  "Design systems that scale globally",
                ].map((t) => (
                  <li key={t} className="flex items-start gap-3">
                    <span className="mt-1 inline-block h-2.5 w-2.5 rounded-full bg-gradient-to-r from-[#115D8E] to-[#046eb4]" />
              ...
              <div className="mt-7 flex flex-wrap gap-2">
                <Chip icon={<ShieldCheck className="h-3.5 w-3.5" />}>Secure by design</Chip>
                <Chip icon={<LineChart className="h-3.5 w-3.5" />}>Measurable impact</Chip>
              </div>
            </GradientCard>
          </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-8">
            <GradientCard compact>
              ...
              <h3 className="mt-5 text-2xl font-extrabold">Turn bold ideas into results</h3>
              <p className="mt-3 text-gray-600">
                We prototype, validate, and launch quickly—then iterate with data and customer insight.
              </p>
            </GradientCard>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { v: "2000+", l: "Distributors" },
                { v: "100,000+", l: "Retailers" },
                { v: "800+", l: "Districts" },
              ].map((k) => (
                <Stat key={k.l} value={k.v} label={k.l} />
              ))}
            </div>
          </div>
        </div>
...
function GradientCard({ children, compact = false }) {
  return (
    <div className="relative rounded-3xl border border-gray-200 bg-white shadow-sm">
      ...
      <div className={`p-6 sm:p-8 ${compact ? "pb-6" : "pb-8"}`}>{children}</div>
      {/* corner glow */}
      <div aria-hidden className="pointer-events-none absolute -right-4 -bottom-4 h-20 w-20 rounded-full bg-gradient-to-tr from-[#115D8E]/15 to-[#046eb4]/15 blur-md" />
    </div>
  );
}
...
function Stat({ value, label }) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-4 text-center shadow-sm">
      ...
      <div aria-hidden className="absolute inset-x-0 -top-px h-0.5 bg-gradient-to-r from-[#115D8E] to-[#046eb4]" />
