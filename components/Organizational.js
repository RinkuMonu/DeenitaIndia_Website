...
import { Crown, Users, Building2, ArrowDown, ArrowUp, Target, Handshake, TrendingUp, ChevronRight, Star, Award, Globe, BarChart3 } from 'lucide-react';

const tabs = [
  {
    id: 1,
    title: "Leadership Structure",
    content: "Our leadership framework ensures clear direction and strategic vision. Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam.",
    icon: Crown,
  ...
  },
  {
    ...
    content: "Our teams are structured for maximum collaboration and efficiency. Content for tab 2: different text can go here. You can add more if needed voluptatem accusantium doloremque laudantium, totam rem aperiam.",
    icon: Users,
  ...
  },
  {
    ...
    content: "Clear department roles ensure accountability and smooth operations. Content for tab 3: replace this with relevant content voluptatem accusantium doloremque laudantium, totam rem aperiam.",
    icon: Building2,
        ...
        <div className="hidden md:block">
          <div className="flex  gap-4 mb-10">
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <div className={`p-3 rounded-full ${activeTab === tab.id ? "bg-[#115D8E]/10" : "bg-gray-100"}`}>
                    <IconComponent className={`w-6 h-6 ${activeTab === tab.id ? "text-[#115D8E]" : "text-gray-500"}`} />
                  </div>
                  {tab.title}
                  <div className={`h-1.5 w-12 rounded-full transition-all duration-500 ${
                    activeTab === tab.id ? "bg-[#115D8E] w-16" : "bg-transparent"
                  }`}></div>
                </button>
              );
            })}
          ...
          >
            {tabs.map((tab) => {
              ...
              if (tab.id === activeTab) {
                return (
                  <div key={tab.id} className="flex flex-col md:flex-row gap-8">
                        ...
                        <div>
                          ...
                          <div className="flex gap-2 mt-3">
                            {tab.features.map((feature, index) => (
                              <span key={index} className="px-3 py-1.5 bg-[#115D8E]/10 text-[#115D8E] rounded-full text-sm font-medium">
                                {feature}
                              </span>
                            ))}
                          </div>
    ...
    return (
      ...
      >
        ...
        >
          ...
          {isOpen ? (
            <ArrowUp className="w-5 h-5 text-[#115D8E]" />
          ) : (
            <ArrowDown className="w-5 h-5 text-[#115D8E]" />
          )}
        </button>
